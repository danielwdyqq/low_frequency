...
\begin{table}
	...
	\begin{tabular}{ll}
		...
		$K$ & Size of \emph{BF}\\
		$C$ & Number of counters in $D$\\
		$N$ & Number of elements in the input stream\\
	    $H$ & Number of hash functions\\
	    $h_i$ & The $ith$ hash function\\
	    $f_S(e)$ & The frequency of element $e$ in $S$\\ 
	    \emph{FPs} &The items with frequency no less than $\lfloor \phi N\rfloor$ wrongly output\\
	    \emph{FNs} & The ttems with frequency less than $\lfloor \phi N\rfloor$ wrongly neglected\\
...
\begin{IEEEproof}
First, a false positive of \emph{BF} means an element not appearing in $S$ but not filtered out by \emph{BF}. Observe that after inserting $M$ keys into a table of size $K$, the probability that a particular bit is still 0 is exactly
\begin{equation} 
(1-\frac{1}{K})^{HM}
\end{equation}
\indent Hence the probability of a false positive in this situation is $(1-(1-\frac{1}{K})^{HM})^H$. However, as we know that $M$ denotes the size of $A$ which is the domain of $S$, so the number of distinct elements in $S$, $M'$ for example, must be no more than $M$, i.e. $M'\leq M$, and further we have $(1-(1-\frac{1}{K})^{HM'})^H\leq (1-(1-\frac{1}{K})^{HM})^H$.
\end{IEEEproof}

\begin{theorem}\label{thm:2}
Assuming no specific data distribution, expectation of number of \emph{FPs} in output of \emph{BFSS} is no more than
\begin{equation}
	(1-(1-\frac{1}{K})^{HM})^H\times M + \frac{1}{\phi}
\end{equation}
\end{theorem}
