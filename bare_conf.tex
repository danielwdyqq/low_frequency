...
Assuming noiseless Pareto data with parameter $\alpha$ and $\beta$, we have:
\begin{eqnarray}\label{eq:9}
	E(\#FPs)<M(1-(1-\frac{1}{K})^{HM})^H + T\\
	T=min(\frac{1}{\phi}, \alpha(1-2^\beta)(\lfloor \phi N\rfloor)^\beta M)
\end{eqnarray}
\end{theorem}

\begin{IEEEproof}
The Pareto distribution has the following property: $Pr(f_S(e)>y)=\alpha y^\beta$, where $\alpha (\alpha \leq 1)$ and $\beta(\beta  <0)$ are parameters of the Pareto distribution. So $Pr(f_S(e)>\lfloor \phi N\rfloor)=\alpha (\lfloor \phi N\rfloor)^\beta$, and the expectation of the number of the elements with $f_S(e)>\lfloor \phi N\rfloor$ is $(\alpha\lfloor \phi N\rfloor)^\beta M'$. In this way, the expectation of the number of the elements with $\lfloor \phi N\rfloor<f_S(e)\leq 2\lfloor \phi N\rfloor$ is $\alpha(1-2^\beta)(\lfloor \phi N\rfloor)^\beta M'$. So the
