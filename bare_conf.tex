...
\begin{algorithmic}[1]
	...
	\STATE $N=0,c=0,C=\lceil 1/\phi\rceil,K=\lambda',H=\mu'$; \COMMENT{$N$: length of stream; $m$: the number of counters used in $D$; $C$: the maximum number of counters in $D$; $K$: size of Bloom filter; $H$: number of hash functions;The value of $\lambda'$ and $\mu'$ will be discussed in detail later.}
...
In this section, we present a theoretical analysis of \emph{BFSS} described in Section \ref{sec:bfss}. We analyze the \emph{FNs} and \emph{FPs}, space complexity, and time complexity. At last, we will identify the existing problems in \emph{BFSS}. \par
...
E(\#FPs)<M(1-(1-\frac{1}{K})^{HM})^H + \lfloor\frac{1}{\phi}\rfloor
\end{equation}
\end{theorem}
...
\subsubsection{\textbf{Analysis of the existing problems }}
In this section, we will identify the existing problems in \emph{BFSS}.\par
From the analysis in section \ref{sec:space} , we observe that $K$ should increase monotonically with $M$ in order to keep a small number of \emph{FPs}, however, what if $M$ is extremely large? Consider such a case: 
