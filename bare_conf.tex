...
\end{IEEEproof}
We may not calculate the exact value of $T$ because we can't get the exact value of $f_S(e_m)$, however, once a query comes, the parameters are confirmed and so is the value of $T$, furthermore, the upper bound of $E(\#FPs)$ is confirmed as well. In this sense, it is meaningful to give the expression of $T$.\par
From Inequation \ref{eq:7}, we observe that the upper bound of $E(\#FPs)$ is related with the values of $H$ and $K$, and our goal is to minimize $E(\#FPs)$ by selecting the appropriate values of them. In addtion, the value of $K$ directly influence the space complexity of \emph{BFSS}, so we will make some analysis of the values of $K$ and $H$ then.
\subsubsection{\textbf{Analysis of the space complexity}}
...
\end{equation}
From above, we know that once the value of $\frac{K}{M}$ is confirmed, we can get an appropriate $H$ to minimize $F$.
\begin{table}
	\centering
   ...
   \begin{tabular}{|c|c|c|c|c|c|c|}
   	...
   	\hline
   	18&12.5&0.000275&0.000226&0.000198&0.000183&0.000176\\
   	\hline
   \end{tabular}
