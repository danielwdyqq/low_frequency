...
SBFSS, which can handle low-frequency items detection over data streams from large domain with acceptable and bounded false negatives and false positives using a proper space.
\end{abstract}
...
\subsubsection{\textbf{Analysis of the space complexity}}\label{sec:space}
...
Like \emph{BFSS}, we will give some theoretical analysis of \emph{SBFSS} in this section, including \emph{FNs}, \emph{FPs}, space complexity, and time complexity.
...
In [], the \emph{SBF} is used to detect duplicates over data streams, in fact, from Alogorithm \ref{alg:sbfss}, we can observe that the function of the \emph{SBF} used in \emph{SBFSS} can also be treated as duplicates detection, in this sense, the properties of the \emph{SBF} used in duplicates detection, including the probability of a false positive and a false negative of \emph{SBF}, are fully applicable to the \emph{SBF} used in \emph{SBFSS}.\par
...
The probability of a false positive of \emph{SBF} is no more than:
\begin{equation}
...
\end{eqnarray}
The tailed derivation of E.q \ref{eq:18} can be found in [], and no more tautology here.\par

\begin{lemma}\label{lem:9}
The expected number of the false negatives of \emph{SBF}, $E(\#FN)$, is: 
\begin{equation}\label{eq:20}
E(\#FN)=M'[1-(1-PR_0(\widehat{\delta},\widehat{p'}))^H]
\end{equation}
where $\widehat{\delta}$ and $\widehat{p'}$ are "average" $\delta$
\end{lemma}
